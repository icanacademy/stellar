...
const REINFORCEMENT_TEMPLATES = [
  ...
  { weakness_area: 'Behavior & Self-Regulation', severity: 'critical', title: 'Behavior Contract & Goals', description: 'Set 3 specific, measurable behavior goals per week. Track daily with simple checkmarks. Reward achievement.', activity_type: 'management', estimated_duration: '5 min/day', dimension_impact_json: JSON.stringify({ RN: -3, AD: -2 }) },
  { weakness_area: 'Behavior & Self-Regulation', severity: 'critical', title: 'Self-Regulation Toolkit', description: 'Learn and practice 3 calming techniques: deep breathing, counting, and positive self-talk. Use when feeling dysregulated.', activity_type: 'exercise', estimated_duration: '5 min as needed', dimension_impact_json: JSON.stringify({ AD: -3, SC: 1 }) },
  { weakness_area: 'Behavior & Self-Regulation', severity: 'significant', title: 'Classroom Expectations Review', description: 'Start each day reviewing classroom rules. End each day self-evaluating adherence with 1-5 rating.', activity_type: 'management', estimated_duration: '5 min/day', dimension_impact_json: JSON.stringify({ RN: -2 }) },
  { weakness_area: 'Behavior & Self-Regulation', severity: 'moderate', title: 'Positive Reinforcement Chart', description: 'Track good behavior moments. After accumulating 10 stars, earn a small reward. Focus on catching positive behaviors.', activity_type: 'management', estimated_duration: 'ongoing', dimension_impact_json: JSON.stringify({ RN: -1 }) },

  // Written Expression
  { weakness_area: 'Written Expression', severity: 'critical', title: 'Daily Handwriting Practice', description: 'Practice letter formation using guided worksheets for 10 minutes daily. Focus on consistency and legibility.', activity_type: 'exercise', estimated_duration: '10 min/day', dimension_impact_json: JSON.stringify({ LG: -3, PT: -2 }) },
  { weakness_area: 'Written Expression', severity: 'critical', title: 'Ergonomic Writing Setup', description: 'Correct pencil grip, posture, and paper angle. Use pencil grips if needed. Ensure proper desk height.', activity_type: 'environment', estimated_duration: 'ongoing', dimension_impact_json: JSON.stringify({ PT: -2 }) },
  { weakness_area: 'Written Expression', severity: 'significant', title: 'Copy Work Practice', description: 'Copy short passages (2-3 sentences) from favorite books. Focus on matching the printed text. Gradually increase length.', activity_type: 'exercise', estimated_duration: '10 min/day', dimension_impact_json: JSON.stringify({ LG: -2, RN: -1 }) },
  { weakness_area: 'Written Expression', severity: 'moderate', title: 'Speed vs Quality Balance', description: 'Practice writing at a slower pace, focusing on quality. Time yourself and gradually increase speed while maintaining legibility.', activity_type: 'exercise', estimated_duration: '10 min/day', dimension_impact_json: JSON.stringify({ AD: -2 }) },
        ...
        const insertMany = database.transaction((reports) => {
          ...
          for (const row of reports) {
            ...
            db.insertDailyReport(database, {
              ...
              is_substitute: row['Is Substitute'] || null,
              skill_focus: row['Skill Focus'] || null,
              sf_met: row['SF Met'] || null,
              current_lesson: row['Current Lesson'] || null,
              materials: row['Materials'] || null,
              homework: row['Homework'] || null,
              next_lesson: row['Next Lesson'] || null,
              ...
              behavior: parseFloat(row['Behavior']) || null,
              handwriting: parseFloat(row['Handwriting']) || null,
              conversation: parseFloat(row['Conversation']) || null,
              skills_json: row['Skills'] || null,
              scores_json: row['Scores'] || null,
              narrative: row['Narrative'] || null,
              wpm_initial: row['WPM Initial'] || null,
              gbwt_initial: row['GBWT Initial'] || null,
              reading_level_initial: row['Reading Level Initial'] || null,
              interview_score: row['Interview Score'] || null
            });
            reportCount++;
          }
