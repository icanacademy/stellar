document.addEventListener('DOMContentLoaded', function() {
            ...
            reportCard.innerHTML = `
                ...
                <div class="report-body">
                    <div class="info-grid-compact">
                        ...
                        ` : ''}
                        <div class="info-item sf-display-item">
                            <span class="label">Skill Focus (SF):</span>
                            <span class="value">${parseSfMet(report['SF Met'], report['Skill Focus']).displayHtml || (report['Skill Focus'] || 'N/A')}</span>
                        </div>
                    </div>
    ...
    function highlightKeywords(text) {
        ...
        const neutralWords = [
            'attention', 'retention', 'comprehension', 'behavior', 'handwriting',
    ...
    function generateLessonPrompt(data) {
        ...
        if (avgRatings.behavior && avgRatings.behavior < 3) {
            performanceWeaknesses.push(`- Behavior: ${avgRatings.behavior}/5 - Behavioral issues affecting learning`);
        }
        if (avgRatings.handwriting && avgRatings.handwriting < 3) {
            performanceWeaknesses.push(`- Handwriting: ${avgRatings.handwriting}/5 - Handwriting quality needs improvement`);
        }
...
FORMAT: Provide the lesson plan in a clear, structured format with time allocations and all necessary materials listed.`;
    ...
    function calculateAcademicFramework(reports) {
        ...
        reports.forEach(report => {
            ...
            totalBehavior += parseFloat(report.Behavior) || 0;
            totalHandwriting += parseFloat(report.Handwriting) || 0;
        ...
        const avgBehavior = totalBehavior / count;
        const avgHandwriting = totalHandwriting / count;
        ...
        // Polishing: Quality (Handwriting + Scores + SF Met)
        const polishing = ((avgHandwriting / 5) * 100 * 0.4) + (avgScore * 0.4) + (avgSkillFocusMet * 0.2);

        // Higher-minds: Metacognition (Behavior - higher behavior = better metacognition)
        const higherminds = (avgBehavior / 5) * 100;

        // Calculate overall performance (average of all 6 ratings)
        const overallPerformance = (avgAttention + avgRetention + avgComprehension + avgBehavior + avgHandwriting + avgConversation) / 6;
        ...
        const stages = [
            {
                ...
                characteristics: [
                    ...
                    'Building foundational academic behaviors',
            ...
            },
            {
                ...
                characteristics: [
                    ...
                    'Improving handwriting and written expression',
            ...
            },
            {
                ...
                characteristics: [
                    ...
                    'Self-regulating behavior and learning independently',
