...
require('dotenv').config({ path: path.join(__dirname, '../online-report-generator/.env') });
    ...
    try {
        ...
        if (USE_SQLITE) {
            ...
            const mapped = rows.map(r => ({
                ...
                Cooperation: r.behavior ? String(r.behavior) : '',
                Engagement: r.handwriting ? String(r.handwriting) : '',
                Conversation: r.conversation ? String(r.conversation) : '',
                Skills: r.skills_json ? (() => { try { return JSON.parse(r.skills_json); } catch(e) { return {}; } })() : {},
                Scores: r.scores_json ? (() => { try { return JSON.parse(r.scores_json); } catch(e) { return {}; } })() : {},
        ...
        fs.createReadStream(CSV_PATH)
            ...
            .on('end', () => {
                ...
                dedupedReports.forEach(report => {
                    // Ratings (CSV columns Behavior/Handwriting map to cooperation/engagement)
                    ...
                    sum.cooperation += parseInt(report.Behavior || report.Cooperation) || 0;
                    sum.engagement += parseInt(report.Handwriting || report.Engagement) || 0;
                    ...
                    const avgRating = (
                        ...
                        (parseInt(report.Behavior || report.Cooperation) || 0) +
                        (parseInt(report.Handwriting || report.Engagement) || 0) +
    ...
    try {
        ...
        fs.createReadStream(CSV_PATH)
            ...
            .on('end', () => {
                ...
                let ratingSum = { attention: 0, retention: 0, comprehension: 0, cooperation: 0, engagement: 0, conversation: 0 };
                const reportsByDate = {};
                ...
                dedupedReports.forEach(report => {
                    ...
                    const ratings = {
                        ...
                        cooperation: parseInt(report.Behavior || report.Cooperation) || 0,
                        engagement: parseInt(report.Handwriting || report.Engagement) || 0,
                    ...
                    if (report.Subject) {
                        if (!subjectPerformance[report.Subject]) {
                            subjectPerformance[report.Subject] = {
                                count: 0,
                                ratingSum: { attention: 0, retention: 0, comprehension: 0, cooperation: 0, engagement: 0, conversation: 0 }
                            };
                        }
                        ...
                        subjectPerformance[report.Subject].ratingSum.cooperation += ratings.cooperation;
                        subjectPerformance[report.Subject].ratingSum.engagement += ratings.engagement;
                ...
                Object.keys(reportsByDate).forEach(date => {
                    ...
                    const dailyAverage = { date, attention: 0, retention: 0, comprehension: 0, cooperation: 0, engagement: 0, conversation: 0 };
                ...
                const subjectComparison = Object.keys(subjectPerformance).map(subject => {
                    ...
                    const avgRating = (
                        ...
                        data.ratingSum.cooperation +
                        data.ratingSum.engagement +
        ...
        fs.createReadStream(CSV_PATH)
            ...
            .on('end', async () => {
                ...
                let ratingSum = {
                    ...
                    cooperation: 0, engagement: 0, conversation: 0
                };
                ...
                Object.keys(reportsByDate).forEach(date => {
                    ...
                    dateReports.forEach(report => {
                        ...
                        dailyAverage.cooperation += report.cooperation;
                        dailyAverage.engagement += report.engagement;
                    ...
                    dailyAverage.cooperation = parseFloat((dailyAverage.cooperation / count).toFixed(2));
                    dailyAverage.engagement = parseFloat((dailyAverage.engagement / count).toFixed(2));
                ...
                // Build AI prompt
                const prompt = `You are an educational analyst specializing in online ESL education. Analyze this student's performance data from online classes and provide actionable insights.

CONTEXT: This student is enrolled in ONLINE classes (not in-person). Ratings reflect online classroom dynamics:
- "Cooperation" measures how well the student collaborates and follows instructions in an online setting
- "Engagement" measures active participation, camera usage, responsiveness, and digital interaction quality
...
- Cooperation: ${aggregatedData.averageRatings.cooperation}
- Engagement: ${aggregatedData.averageRatings.engagement}
...
Component Breakdown (Online Learning):
1. Test Scores (40% weight): ${academicPerf.components.testScores.score}/100
...
2. Skill Focus Met Rate (20% weight): ${academicPerf.components.skillFocusMet.score}/100
   - Met ${academicPerf.components.skillFocusMet.met} out of ${academicPerf.components.skillFocusMet.total} sessions

3. Online Classroom Participation (40% weight): ${academicPerf.components.participation.score}/100
   - This is from the framework scores (Attention, Retention, Cooperation, Engagement, etc.)
`;
        }

        const prompt = `You are an educational development specialist analyzing an ONLINE student's performance data. This student attends classes remotely via video call.

STUDENT: ${student}
GENDER: ${studentGender} (use pronouns: ${genderPronoun})
ENROLLED GRADE: Grade ${actualGrade}
LEARNING MODE: Online (remote via video call)
...
=== ONLINE CLASSROOM PARTICIPATION SCORES (SECONDARY - 10% of overall) ===
...
- Polishing: ${frameworkData.polishing}/100 - Quality & creativity (Engagement + Test Scores + Skill Focus Met)
- Higher-minds: ${frameworkData.higherminds}/100 - Metacognition (Cooperation & self-regulation)

NOTE: This is an ONLINE student. "Cooperation" reflects collaboration in virtual settings. "Engagement" reflects digital participation quality (camera on, responsiveness, interaction).

=== INTERPRETATION GUIDE ===
Academic Score Thresholds:
- 85+ = Above grade level (tests and assessments exceed Grade ${actualGrade} expectations)
...
${academicPerf ? `Based on ACTUAL academic indicators (test scores, assessments), this student is performing ${academicPerf.gradeComparison === 'above' ? 'ABOVE' : academicPerf.gradeComparison === 'at' ? 'AT' : academicPerf.gradeComparison === 'approaching' ? 'SLIGHTLY BELOW' : 'BELOW'} Grade ${actualGrade} expectations.` : ''}

CRITICAL INSTRUCTIONS:
- Use the ACADEMIC PERFORMANCE INDICATORS as your primary assessment (if available)
- Online classroom participation scores are SECONDARY - good attention doesn't mean above-grade academics
...
- Consider online learning challenges (screen fatigue, home distractions, internet connectivity)
...
function analyzePersonaPerformanceTier(analytics, chartData) {
  ...
  const metricsAvg = ['attention', 'retention', 'comprehension', 'cooperation', 'engagement', 'conversation']
  ...
  const lowestMetric = Math.min(
    ...
    parseFloat(avgRatings.cooperation) || 5,
    parseFloat(avgRatings.engagement) || 5,
...
function analyzeGaps(metrics, skillProfile, performanceTier) {
  ...
  const metricLabels = {
    ...
    behavior: 'Cooperation & Self-Regulation',
    handwriting: 'Engagement & Participation',
  ...
  const impacts = {
    ...
    behavior: 'May affect online classroom cooperation and peer relationships',
    handwriting: 'Can impact digital engagement, responsiveness, and active participation',
...
app.get('/api/student-persona/:studentName', async (req, res) => {
  try {
    ...
    const metrics = {
      ...
      behavior: parseFloat(analytics.averageRatings?.cooperation) || 3,
      handwriting: parseFloat(analytics.averageRatings?.engagement) || 3,
...
- Cooperation: ${persona.metrics.behavior.toFixed(1)}
- Engagement: ${persona.metrics.handwriting.toFixed(1)}
...
- Cooperation: ${snapshot.avg_behavior}
- Engagement: ${snapshot.avg_handwriting}
...
app.post('/api/reports/import', (req, res) => {
  try {
    ...
    dbModule.insertDailyReport(sqliteDb, {
      ...
      behavior: parseFloat(report['Cooperation']) || null,
      handwriting: parseFloat(report['Engagement']) || null,
...
// ============================================
// RESYNC - Full CSV re-import for Data Manager
// ============================================

app.post('/api/reports/resync', (req, res) => {
  try {
    if (!sqliteDb) return res.status(503).json({ error: 'SQLite not available' });

    // Clear existing data (order respects FK constraints)
    sqliteDb.exec('DELETE FROM reinforcement_recommendations');
    sqliteDb.exec('DELETE FROM projected_persona');
    sqliteDb.exec('DELETE FROM snapshot_weaknesses');
    sqliteDb.exec('DELETE FROM persona_snapshots');
    sqliteDb.exec('DELETE FROM desired_persona');
    sqliteDb.exec('DELETE FROM attendance_overrides');
    sqliteDb.exec('DELETE FROM academic_enabler');
    sqliteDb.exec('DELETE FROM quiz_data');
    sqliteDb.exec('DELETE FROM daily_reports');
    sqliteDb.exec('DELETE FROM students');

    // Re-read CSV
    if (!fs.existsSync(CSV_PATH)) {
      USE_SQLITE = false;
      return res.json({ success: true, imported: 0, total: 0, deduped: 0 });
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (row['Student Name'] && row['Date']) rows.push(row);
    }

    // Deduplicate by Student Name + Date + Subject (keep last)
    const deduped = new Map();
    rows.forEach(r => {
      const key = `${r['Student Name']}|${r['Date']}|${r['Subject']}`;
      deduped.set(key, r);
    });

    const uniqueRows = Array.from(deduped.values());
    const dedupedCount = rows.length - uniqueRows.length;
    let imported = 0;

    const importTransaction = sqliteDb.transaction((reportRows) => {
    ...
    importTransaction(uniqueRows);
    USE_SQLITE = imported > 0;

    console.log(`Resync complete: ${imported} imported, ${dedupedCount} deduped`);
    res.json({ success: true, imported, total: rows.length, deduped: dedupedCount });
  } catch (error) {
    console.error('Error during resync:', error);
    res.status(500).json({ error: 'Resync failed: ' + error.message });
  }
});

app.listen(port, '0.0.0.0', () => {
    console.log(`Online Student Report Viewer running at http://localhost:${port}`);
