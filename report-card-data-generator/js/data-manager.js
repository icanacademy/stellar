/**
 * Data Manager — CSV admin interface for report data.
 */
document.addEventListener('DOMContentLoaded', function () {
    // ── Auth Gate ──
    var dmToken = sessionStorage.getItem('dm_token') || '';
    ...
    document.getElementById('authForm').addEventListener('submit', function (e) {
        ...
        fetch('/api/dm-auth', {
            ...
            .then(function (data) {
                if (data.success) {
                    dmToken = data.token;
                    sessionStorage.setItem('dm_token', dmToken);
                    hideAuthOverlay();
                    initApp();
                } else {
    ...
    function renderBrowseTable(rows) {
        ...
        tbody.innerHTML = rows.map(function (r) {
            var sfMet = r['SF Met'] || '';
            var sfClass = '';
            if (sfMet === 'YES') sfClass = 'color:#16a34a;font-weight:700;';
            else if (sfMet === 'NO') sfClass = 'color:#dc2626;font-weight:700;';
            else if (sfMet.startsWith('{')) sfClass = 'color:#d97706;font-weight:700;';
            ...
            return '<tr>' +
                ...
                '<td>' + esc(r.Subject) + '</td>' +
                '<td>' + esc(truncate(r['Skill Focus'], 20)) + '</td>' +
                '<td style="' + sfClass + '">' + esc(sfMet.length > 6 ? 'Mixed' : sfMet) + '</td>' +
    ...
    var editFieldOrder = ['Date', 'Day of Week', 'Student Name', 'Student ID', 'Grade Level',
        'WPM Initial', 'GBWT Initial', 'Reading Level Initial', 'Interview Score',
        'Teacher Name', 'Teacher ID', 'Subject', 'Is Substitute',
        'Skill Focus', 'SF Met', 'Current Lesson', 'Materials', 'Homework', 'Next Lesson',
    ...
    var longFields = ['Activities Finished', 'Activities Not Finished', 'Skills', 'Scores', 'Narrative', 'Current Lesson', 'Homework', 'Next Lesson'];
            ...
            .then(function (data) {
                ...
                container.innerHTML = data.groups.map(function (group) {
                    ...
                    var tableHtml = '<table><thead><tr>' +
                        '<th>Index</th><th>Teacher</th><th>Skill Focus</th><th>Attention</th><th>Narrative (preview)</th><th>Action</th>' +
                        '</tr></thead><tbody>' +
                        group.rows.map(function (r) {
                            return '<tr>' +
                                '<td>' + r._index + '</td>' +
                                '<td>' + esc(truncate(r['Teacher Name'], 20)) + '</td>' +
                                '<td>' + esc(truncate(r['Skill Focus'], 15)) + '</td>' +
    ...
    document.getElementById('btnAutoDedup').addEventListener('click', function () {
        showConfirm('Remove All Duplicates',
            'This will remove all duplicate rows, keeping only the first occurrence. A backup will be created. Continue?',
            function () { performDedup(); }
        );
    });
